...
<form method="post" action="{$F_ACTION}">
  ...
  <p><label><input type="checkbox" name="user_albums"{if $user_albums} checked="checked"{/if}> <strong>{'User albums'|translate}</strong> : <em>{'Phyxo automatically creates an album for each user, on first connection'|translate}</em></label></p>

  <p class="subOption"{if !$user_albums} style="display:none;"{/if} id="userAlbumsLocation">
    <strong>{'Where should Phyxo create user albums?'|translate}</strong>
    <br>
